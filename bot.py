...
THINKING_FRAMES = ["⏺", "⏺⏺", "⏺⏺⏺"]
THINKING_FRAME_DELAY = 0.25
TYPEWRITER_STEPS = 4
TYPEWRITER_STEP_DELAY = 0.15
...
async def send_thinking_reply(
    ...
    msg = await context.bot.send_message(chat_id=chat_id, text=THINKING_FRAMES[0])
    clean = text or ""
    try:
        for frame in THINKING_FRAMES[1:]:
            await asyncio.sleep(THINKING_FRAME_DELAY)
            await _safe_edit_text(context, chat_id, msg.message_id, frame)

        if clean and len(clean) <= TYPEWRITER_MAX_CHARS:
            await asyncio.sleep(THINKING_FRAME_DELAY)
            for step in range(1, TYPEWRITER_STEPS + 1):
                cut = max(1, round(len(clean) * step / TYPEWRITER_STEPS))
                partial = clean[:cut] + ("▍" if cut < len(clean) else "")
                await _safe_edit_text(context, chat_id, msg.message_id, partial)
...
async def send_thinking_photo(
    ...
    msg = await context.bot.send_photo(chat_id=chat_id, photo=photo, caption=THINKING_FRAMES[0])
    clean = caption or ""
    try:
        for frame in THINKING_FRAMES[1:]:
            await asyncio.sleep(THINKING_FRAME_DELAY)
            await _safe_edit_caption(context, chat_id, msg.message_id, frame)
        ...
        if clean and len(clean) <= TYPEWRITER_MAX_CHARS:
            ...
            for step in range(1, TYPEWRITER_STEPS + 1):
                cut = max(1, round(len(clean) * step / TYPEWRITER_STEPS))
                partial = clean[:cut] + ("▍" if cut < len(clean) else "")
                await _safe_edit_caption(context, chat_id, msg.message_id, partial)
                await asyncio.sleep(TYPEWRITER_STEP_DELAY)
        elif clean:
            await asyncio.sleep(THINKING_FRAME_DELAY)
    except Exception:
    ...
    if _text_mentions_bot(message, bot.username) or _is_reply_to_bot(message, bot.id):
        ...
        await bot.send_message(
            ...
            text=(
                ...
                "Sebelum lanjut, klik dulu tombol *Mulai* di bawah ini ya 👇"
            ),
            parse_mode="Markdown",
...
async def group_start_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    total_talent = len(db.list_talents())
    guide_lines = [
        f"✨ *Halo, aku {config.BOT_NAME}!*",
        "",
        f"Aku bantu kamu lihat *{total_talent} talent* yang tersedia, cek pricelist, "
        "sampai mulai order -- semua bisa langsung dari grup ini 😉",
        "",
        "*Coba ketik salah satu ini:*",
        "• `order talent` — lihat katalog talent",
        "• `katalog` — sama saja, lihat daftar talent",
        "",
        "_Catatan: fitur 💬 Chat Sekarang (ngobrol langsung dengan admin) hanya "
        "bisa dipakai lewat chat pribadi denganku, bukan di grup._",
    ]
    await send_typing(context, chat.id)
    await context.bot.send_message(
        chat_id=chat.id,
...
async def relay_user_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_first:
            ...
            # Format ala livechatgram: foto profil user kecil di kiri + nama
            # (bisa diklik, langsung buka profil user lewat tg://user?id=...),
            # isi pesan digabung di bawahnya sebagai caption foto tersebut.
            mention = _mention_html(user.full_name, user.id)
            profile_photo = await _get_profile_photo_file_id(context, user.id)
            ...
            if message.text and not message.caption:
                ...
                if profile_photo:
                    targets = await broadcast_photo_to_admin_targets(
                        context, profile_photo, caption_body, parse_mode="HTML",
                    )
                    # Fallback ke pesan teks biasa kalau kirim foto gagal, mis.
                    # caption kepanjangan (limit caption Telegram 1024 karakter).
                    if not targets:
                        targets = await broadcast_to_admin_targets(context, caption_body, parse_mode="HTML")
                else:
                    targets = await broadcast_to_admin_targets(context, caption_body, parse_mode="HTML")
            else:
                # Pesan berupa media -> kirim "header" (foto profil + mention)
                # dulu sebagai bubble terpisah, baru teruskan media aslinya apa
                # adanya (mengikuti pola yang sama seperti pesan pertama).
                if profile_photo:
                    header_targets = await broadcast_photo_to_admin_targets(
                        context, profile_photo, mention, parse_mode="HTML",
                    )
                else:
