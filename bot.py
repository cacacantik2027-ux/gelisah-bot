...
def _admin_commands_list(public_commands):
    ...
    return public_commands + [
        ...
        BotCommand("setabsentopik", "Atur topik Absen Talent (jalankan di topiknya)"),
...
# ==================== FITUR ABSEN TALENT ====================
# Talent absen di topik "Absen Talent" pada grup private talent (diatur
# lewat /setabsentopik) dengan pesan bebas semacam "Aca ready sampe jam 2"
# atau "Aca off". Bot mendeteksi nama talent yang disebut (atau nama
# Telegram si pengirim sebagai fallback) + status ready/tidak, lalu
# menyimpannya ke tabel talent_status. Status ini otomatis balik jadi
# TIDAK READY kalau jam ready-nya sudah lewat (lihat _check_talent_status_expiry).
...
def _match_talent_from_text(text_lower: str, talents):
    ...
    nama pendek mis. 'aca' tidak nyangkut ke kata lain yang kebetulan
    mengandungnya, mis. 'kacamata')."""
    for t in talents:
...
def _match_talent_from_sender(user, talents):
    """Fallback kalau nama talent tidak disebut eksplisit di teks -- cocokkan
    first_name/username pengirim Telegram ke daftar nama talent."""
    if not user:
        return None
    candidates = [c.strip().lower() for c in (user.first_name, user.username) if c]
    if not candidates:
        return None
    for t in talents:
        name = (t["name"] or "").strip().lower()
...
async def absen_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Deteksi pesan absen di topik "Absen Talent" (grup private talent,
    lihat /setabsentopik). Kalau topik absen belum diatur, ATAU pesan ini
    bukan dari grup+topik yang dikonfigurasi, ATAU teksnya tidak dikenali
    sebagai format absen -- bot diam saja (tidak balas apa pun), supaya
    obrolan santai lain tidak ikut kepicu."""
    message = update.effective_message
    absen_group = db.get_setting("absen_group_id")
    absen_topic = db.get_setting("absen_topic_id")
    if not absen_group or not absen_topic:
        return
    if str(update.effective_chat.id) != str(absen_group):
        return
    if str(message.message_thread_id or "") != str(absen_topic):
        return
    ...
    talents = db.list_talents()
    talent = _match_talent_from_text(text.lower(), talents) or _match_talent_from_sender(update.effective_user, talents)
    if not talent:
        await message.reply_text(
            "⚠️ Nama talent tidak dikenali dari pesan ini. Sebutkan nama talent-mu di pesan, "
            "mis. \"Aca ready sampe jam 2\".",
    ...
    db.set_talent_status(talent["id"], is_ready, ready_until_utc_str, text, update.effective_user.id)
...
async def _check_talent_status_expiry(context: ContextTypes.DEFAULT_TYPE):
    ...
    otomatis flip ke TIDAK READY, lalu kabari di topik Absen Talent supaya
    ...
    for talent_id in expired_ids:
        ...
        try:
            await context.bot.send_message(
                chat_id=int(absen_group),
                text=f"⏰ *{name}* otomatis jadi TIDAK READY (jam ready sudah lewat).",
                parse_mode="Markdown",
                message_thread_id=int(absen_topic) if absen_topic else None,
            )
        except Exception:
            logger.exception("Gagal kirim notifikasi auto-expire status talent #%s", talent_id)


async def setabsentopik_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Jalankan DI DALAM topik "Absen Talent" pada grup private talent
    (khusus admin) untuk mendaftarkan topik itu sebagai sumber absen.
    Pakai: `/setabsentopik hapus` untuk melepas pengaturan."""
    ...
    if context.args and context.args[0].lower() == "hapus":
        ...
        await update.message.reply_text("✅ Pengaturan topik Absen Talent sudah dilepas.")
        return
    ...
    if not thread_id:
        await update.message.reply_text(
            ...
            "topik \"General\"). Aktifkan Topics di pengaturan grup, buat topik \"Absen Talent\", lalu "
            "jalankan command ini DI DALAM topik itu."
        )
        return

    db.set_setting("absen_group_id", str(update.effective_chat.id))
    db.set_setting("absen_topic_id", str(thread_id))
    await update.message.reply_text(
        "✅ Topik ini diatur sebagai topik *Absen Talent*.\n\n"
        "Mulai sekarang, pesan absen di sini (mis. \"Aca ready sampe jam 2\" atau \"Aca off\") "
        "akan otomatis dideteksi & status talent-nya diperbarui.",
        parse_mode="Markdown",
    )
...
async def _prepare_testimoni_photo(context: ContextTypes.DEFAULT_TYPE, file_id: str):
    ...
    if censor_on:
        if _testimoni_censor_available():
            try:
                image_bytes = await asyncio.to_thread(_redact_sensitive_regions, image_bytes)
            except Exception:
...
async def _post_testimoni_photo(context: ContextTypes.DEFAULT_TYPE, channel_raw: str, file_id: str, talent_name: str):
    ...
    caption = f"✅ Testimoni order untuk *{talent_name}*\n\n#testimoni"
...
async def testimoni_approve_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.answer(f"❌ Gagal posting ke channel testimoni: {e}", show_alert=True)
        return

    await query.answer("✅ Berhasil diposting ke channel testimoni!")
...
def main():
    ...
    settings_conv = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(
                settings_callback,
                pattern="^settings_|^delconfirm_|^sponsor|^edittalent|^editsponsor|^editadmin",
            ),
        ],
        states={
            ADD_NAME: [
    ...
    if app.job_queue:
        ...
        logger.warning(
            ...
            "auto-off proaktif di topik Absen Talent tidak akan jalan. Install dengan: "
            "`pip install \"python-telegram-bot[job-queue]\"` kalau mau mengaktifkannya."
        )
