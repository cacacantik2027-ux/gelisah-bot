...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("acc_") or data.startswith("rej_"):
        ...
        if action == "acc":
            # Ambil file_id foto bukti transfer SEBELUM caption diedit
            ...
            # Auto-forward ke channel jika channel_id sudah diatur
            ...
            if channel_id != 0:
                try:
                    ...
                    caption_testi = (
                        f"✅ *TESTIMONI {talent_name}*\n\n"
                        "Order Sekarang : @ordervcsgs_bot"
                    )

                    # Proses foto bukti transfer (dengan watermark jika ada)
                    ...
                    # Kirim sebagai album (media group) — foto talent + foto bukti transfer
                    if proof_media and talent_photo:
                        ...
                        # Hanya foto bukti transfer (talent tidak punya foto)
                        await ctx.bot.send_photo(
                            chat_id=channel_id,
                            photo=proof_media.media,
                            caption=caption_testi,
                            parse_mode=ParseMode.MARKDOWN,
                        )
                    elif talent_photo:
                        # Hanya foto talent (bukti transfer tidak terdeteksi)
                        await ctx.bot.send_photo(
                            chat_id=channel_id,
                            photo=talent_photo,
                            caption=caption_testi,
