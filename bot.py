...
# ==================== Animasi "thinking dots" + typewriter (ala Claude) ====================
# Dipakai sebagai pengganti send_message/send_photo biasa di alur-alur utama
# bot, supaya balasan bot terasa "hidup" seperti chat AI:
#   1) THINKING DOTS -- pesan sementara berisi titik yang bertambah, di-edit
#      beberapa kali dengan cepat (meniru animasi "sedang berpikir").
#   2) TYPEWRITER -- pesan yang sama lalu di-edit bertahap mengungkap teks
#      jawaban asli sedikit demi sedikit dengan kursor "▍" di ujungnya,
#      sebelum berhenti di edit TERAKHIR berisi teks FINAL yang sudah
#      diformat lengkap (Markdown/entities + tombol).
#
# Semua edit di TENGAH animasi SENGAJA dikirim polos (parse_mode=None) --
# potongan Markdown yang belum lengkap (mis. "*kata" tanpa penutup bintang)
# akan ditolak Telegram ("can't parse entities"). Cuma edit TERAKHIR yang
# memakai parse_mode/entities/tombol yang sesungguhnya.
#
# THINKING DOTS: "." -> ".." -> "..." lalu berulang, persis seperti indikator
# "sedang berpikir" di claude.ai/DM Claude, tampil selama kurang lebih
# THINKING_DOTS_TOTAL_SECONDS detik sebelum lanjut ke tahap typewriter.
THINKING_DOTS_FRAMES = [".", "..", "..."]
THINKING_DOTS_FRAME_DELAY = 0.4
THINKING_DOTS_TOTAL_SECONDS = 2.0

# TYPEWRITER: setelah thinking dots selesai, teks jawaban diungkap bertahap
# (bukan langsung utuh) supaya terasa "sedang diketik" ala Claude. Jumlah
# langkah menyesuaikan panjang teks -- teks pendek cukup sedikit langkah,
# teks panjang dapat lebih banyak langkah (lebih halus) tapi dibatasi supaya
# tidak memicu flood limit Telegram gara-gara kebanyakan edit pesan.
TYPEWRITER_STEP_DELAY = 0.12
TYPEWRITER_MIN_STEPS = 6
TYPEWRITER_MAX_STEPS = 18
# Teks lebih panjang dari ini langsung dikirim utuh tanpa typewriter (cuma
# thinking dots) -- supaya balasan panjang (mis. pricelist) tidak kelamaan
# tampil & tidak memicu flood limit Telegram gara-gara kebanyakan edit.
TYPEWRITER_MAX_CHARS = 500


def _typewriter_steps_for(length: int) -> int:
    """Tentukan berapa langkah pengungkapan teks (chunk) dipakai untuk teks
    sepanjang `length` karakter -- makin panjang teksnya, makin banyak
    langkahnya (lebih halus terasa mengetik), dibatasi MIN/MAX_STEPS."""
    if length <= 0:
        return 0
    return max(TYPEWRITER_MIN_STEPS, min(TYPEWRITER_MAX_STEPS, length // 8))
...
async def _run_typewriter(edit_fn, clean: str):
    """Ungkap `clean` bertahap (dengan kursor "▍" di ujung tiap potongan yang
    belum final) lewat `edit_fn(text)`, jumlah langkah menyesuaikan panjang teks."""
    steps = _typewriter_steps_for(len(clean))
    for step in range(1, steps + 1):
        cut = max(1, round(len(clean) * step / steps))
        partial = clean[:cut] + ("▍" if cut < len(clean) else "")
        await edit_fn(partial)
        await asyncio.sleep(TYPEWRITER_STEP_DELAY)
...
async def send_thinking_reply(
    ...
    """Kirim balasan TEKS dengan animasi thinking dots + typewriter ala Claude,
    berhenti di teks FINAL yang sudah diformat lengkap."""
    try:
        await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
    except Exception:
        pass

    msg = await context.bot.send_message(chat_id=chat_id, text=THINKING_DOTS_FRAMES[0])
    clean = text or ""
    edit_fn = lambda t: _safe_edit_text(context, chat_id, msg.message_id, t)
    try:
        await _run_thinking_dots(edit_fn)

        if clean and len(clean) <= TYPEWRITER_MAX_CHARS:
            await _run_typewriter(edit_fn, clean)
    except Exception:
        logger.debug("Animasi thinking/typewriter berhenti di tengah jalan (diabaikan).")
    ...
    try:
        ...
        logger.exception("Gagal edit final pesan thinking/typewriter, fallback hapus+kirim ulang.")
        try:
...
async def send_thinking_photo(
    ...
    bisa 'di-streaming'), tapi CAPTION-nya dianimasikan thinking dots +
    typewriter sebelum berhenti di caption FINAL yang sudah diformat lengkap."""
    try:
        await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
    except Exception:
        pass

    msg = await context.bot.send_photo(chat_id=chat_id, photo=photo, caption=THINKING_DOTS_FRAMES[0])
    clean = caption or ""
    ...
    try:
        ...
        logger.debug("Animasi thinking/typewriter (foto) berhenti di tengah jalan (diabaikan).")
    ...
    try:
        ...
        logger.exception("Gagal edit final caption thinking/typewriter, fallback hapus+kirim ulang.")
        try:
            await context.bot.delete_message(chat_id=chat_id, message_id=msg.message_id)
        except Exception:
            pass
...
async def replace_message(query, context, text, reply_markup=None, parse_mode=None, photo=None, entities=None):
    ...
    pesan baru sebagai gantinya, dengan animasi 'thinking dots' + efek
    streaming/typewriter ala Claude sebelum teks final muncul -- supaya
...
async def send_talent_card_to_chat(context, chat_id, talents, index, close_button=False):
    ...
    (mis. dari smart-reply grup saat user mengetik "order talent")."""
    total = len(talents)
    index = max(0, min(index, total - 1))
    talent = talents[index]

    caption = f"*{talent['name']}*"
    reply_markup = kb.talent_carousel_keyboard(talent, index, total, close_button=close_button)

    if talent.get("photo_file_id"):
        await send_thinking_photo(
            context, chat_id, talent["photo_file_id"], caption,
...
async def show_talent_card(query, context, talents, index):
    ...
    grup (termasuk saat navigasi Sebelumnya/Selanjutnya ditekan di grup),
    "⬅️ Kembali" kalau di private chat -- konsisten dengan kartu awal yang
    dikirim send_talent_card_to_chat dari smart-reply grup."""
    await delete_prev_message(query, context)
    close_button = query.message.chat.type != ChatType.PRIVATE
    await send_talent_card_to_chat(context, query.message.chat_id, talents, index, close_button=close_button)
...
async def show_talent_detail(context: ContextTypes.DEFAULT_TYPE, chat_id, talent):
    ...
    if talent.get("photo_file_id"):
        await send_thinking_photo(
            context, chat_id, talent["photo_file_id"], caption,
            parse_mode="Markdown", reply_markup=kb.talent_detail_keyboard(talent),
        )
    else:
        await send_thinking_reply(
            context, chat_id, caption,
...
# Fitur baru: kalau bot di-mention (@username_bot) atau di-reply di dalam
# grup, bot langsung membalas pesan itu (reply_to_message_id) dengan tombol
# "🚀 Mulai". Begitu user yang sama menekannya, bot mengirim panduan singkat
# (inti kegunaan bot + contoh perintah) -- SEMUA masih di dalam grup yang
# sama, TANPA perlu pindah ke chat pribadi/interface bot. Setelah itu, kalau
# user mengetik kalimat seperti "order talent" atau "katalog", bot membalas
# lagi dengan tombol katalog talent (menu utama).
#
...
#
# Catatan penyimpanan: status "user ini sudah klik Mulai di grup ini" disimpan
# di context.chat_data (memori proses, per grup) -- cukup untuk kebutuhan
# fitur ini dan tidak perlu tabel database baru. Efeknya, status ini reset
# kalau bot di-restart/redeploy; itu wajar & tidak masalah karena user tinggal
# mention ulang atau klik Mulai lagi.

ORDER_INTENT_KEYWORDS = ("order", "pesan", "booking", "mau order", "mau pesan")
...
def _matches_order_intent(text: str) -> bool:
    """Deteksi 'pintar' (sederhana, berbasis kata kunci) untuk niat order,
    supaya tidak harus persis "order talent" -- juga menangkap variasi
    seperti "mau order talent", "pesan talent dong", atau cukup "katalog"."""
    t = (text or "").lower()
    if "katalog" in t:
        return True
    if "talent" in t and any(k in t for k in ORDER_INTENT_KEYWORDS):
        return True
    return False


async def group_smart_reply_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handler utama fitur smart-reply di grup. Sengaja HANYA bereaksi kalau
    (a) bot di-mention/di-reply, atau (b) user yang bersangkutan sudah pernah
    klik tombol "Mulai" di grup ini DAN pesannya cocok niat order/katalog --
    supaya bot tidak ikut membalas obrolan grup yang tidak ada urusannya
    dengan bot sama sekali."""
    ...
    bot = context.bot
    started_users = context.chat_data.setdefault("mention_started_users", set())

    if _text_mentions_bot(message, bot.username) or _is_reply_to_bot(message, bot.id):
        ...
        await bot.send_message(
            ...
            text=(
                f"Halo {mention}! 👋\n\n"
                "Sebelum lanjut, klik dulu tombol <b>Mulai</b> di bawah ini ya 👇"
            ),
            parse_mode="HTML",
            reply_to_message_id=message.message_id,
            reply_markup=kb.group_start_keyboard(user.id),
        )
        return

    if user.id in started_users and _matches_order_intent(message.text):
        talents = db.list_talents()
        if not talents:
            await send_typing(context, chat.id)
            await bot.send_message(
        ...
        await message.reply_text("💃 Ini dia katalog talent kami:")
        await send_talent_card_to_chat(context, chat.id, talents, 0, close_button=True)
        return
...
async def group_start_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tombol yang sama tidak bisa "dibajak"/dipakai orang lain di grup itu."""
    query = update.callback_query
    try:
    ...
    started_users = context.chat_data.setdefault("mention_started_users", set())
    started_users.add(target_user_id)
    ...
    # dikirim sebagai pesan TERSENDIRI dulu, baru disusul pesan panduan teks
    # di bawahnya ("pesan berikutnya"). Kalau belum diatur admin, langsung
    # lanjut ke pesan panduan seperti biasa (tidak wajib).
    await send_group_start_greeting_media(context, chat.id)

    total_talent = len(db.list_talents())
    mention = _mention_html(query.from_user.full_name, query.from_user.id)
    guide_lines = [
        f"✨ Halo, {mention}! Aku <b>{html_escape(config.BOT_NAME)}</b>.",
        "",
        f"Aku bantu kamu lihat <b>{total_talent} talent</b> yang tersedia, cek pricelist, "
        "sampai mulai order -- semua bisa langsung dari grup ini 😉",
        "",
        "<b>Coba ketik salah satu ini:</b>",
        "• <code>order talent</code> — lihat katalog talent",
        "• <code>katalog</code> — sama saja, lihat daftar talent",
        "",
        "<i>Catatan: fitur 💬 Chat Sekarang (ngobrol langsung dengan admin) hanya "
        "bisa dipakai lewat chat pribadi denganku, bukan di grup.</i>",
    ]
    await send_typing(context, chat.id)
    await context.bot.send_message(
        chat_id=chat.id,
...
async def reset_chat_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not sessions:
        ...
        for s in sessions:
            lines.append(f"#{s['id']} - {s['talent_name']} - {s['full_name']} (@{s['username']})")
        await replace_message(
            query, context, "\n".join(lines), parse_mode="Markdown",
...
async def settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "settings_sessions":
        ...
        lines.append(
            "\n_Kalau ada user yang mengeluh terjebak/tidak dibalas, tekan tombol "
            "♻️ Reset di bawah untuk sesi terkait -- sesi itu akan ditutup & "
            "dibersihkan supaya user bisa langsung memulai live chat yang baru._"
        )
        text = "\n".join(lines)
        await replace_message(query, context, text, parse_mode="Markdown", reply_markup=kb.active_sessions_keyboard(sessions))
        return ConversationHandler.END
...
def main():
    ...
    # atau kata kunci order, bukan sembarang pesan grup.
