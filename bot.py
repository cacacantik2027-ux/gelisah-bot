...
def _admin_commands_list(public_commands):
    ...
    return public_commands + [
        ...
        BotCommand("linktalent", "Tautkan akun talent: /linktalent @username Nama Talent"),
...
def _link_talent(user_id, talent_id):
...
async def _resolve_telegram_username(context: ContextTypes.DEFAULT_TYPE, username: str):
    """Resolve @username Telegram jadi (user_id, label_tampilan) lewat
    Bot API getChat -- INI BEKERJA WALAU USER-NYA BELUM PERNAH CHAT/DIKENAL
    bot sama sekali sebelumnya, selama username-nya publik & valid (beda
    dari cara "reply pesan talent" yang mewajibkan ada jejak pesan lebih
    dulu). Balikin (None, None) kalau gagal resolve atau ternyata bukan
    akun pengguna biasa (mis. malah username grup/channel)."""
    try:
        chat = await context.bot.get_chat(username if username.startswith("@") else f"@{username}")
    except Exception:
        logger.warning("Gagal resolve username Telegram %r lewat getChat.", username)
        return None, None
    if chat.type != ChatType.PRIVATE:
        return None, None
    label = f"@{chat.username}" if chat.username else (chat.first_name or username)
...
async def linktalent_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    1. Langsung sebut usernamenya, TANPA perlu reply pesan apa pun:
       ...
       (resolve lewat Bot API getChat -- bekerja walau talent itu belum
       pernah kirim pesan apa pun yang bisa di-reply, asal username-nya
       publik & valid.)
    2. Reply pesan APAPUN dari talent yang bersangkutan, lalu:
         `/linktalent Nama Talent`
       (fallback kalau cara 1 gagal resolve, mis. karena alasan privasi
       akun tertentu.)
    ...
    if args and args[0].startswith("@"):
        ...
        if target_id is None:
            await update.message.reply_text(
                f"⚠️ Tidak bisa menemukan user dengan username {username} (mungkin salah ketik, "
                "usernya belum punya username publik, atau baru saja ganti username).\n\n"
                "Alternatif: reply salah satu pesan DARI talent tsb, lalu jalankan "
                f"`/linktalent {name_query}` (tanpa @username).",
                parse_mode="Markdown",
            )
            return
        ...
        _link_talent(target_id, talent["id"])
        await update.message.reply_text(
    ...
    _link_talent(target.id, talent["id"])
...
def _build_promo_message(bot_username):
    ...
    lines = ["🔥 *Talent Ready Sekarang!*", ""]
    for talent, status in ready:
        until = status.get("ready_until")
        if until:
            local = datetime.strptime(until, "%Y-%m-%d %H:%M:%S").replace(tzinfo=UTC_TZ).astimezone(WIB_TZ)
            lines.append(f"🟢 *{talent['name']}* -- ready sampai {local.strftime('%H:%M')} WIB")
        else:
            lines.append(f"🟢 *{talent['name']}*  -- ready")
    lines.append("")
    lines.append("Tekan tombol nama talent di bawah untuk langsung chat lewat bot 👇")
    text = "\n".join(lines)
