...
async def show_talent_card(query, context, talents, index):
    ...
    caption = f"*{talent['name']}*"
    reply_markup = kb.talent_carousel_keyboard(talent, index, total, close_button=close_button)
    photo = talent.get("photo_file_id")
    ...
    await delete_prev_message(query, context)
    await send_talent_card_to_chat(context, chat.id, talents, index, close_button=close_button)
...
async def talent_detail_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await delete_prev_message(query, context)
    await show_talent_detail(context, chat.id, talent)
...
# Admin utama (config.ADMIN_IDS, dari server) bisa menambahkan admin lain
# lewat /addadmin supaya bisa ikut mengelola & membalas chat pengguna saat
# live chat aktif -- tanpa perlu ubah environment variable / redeploy.
# Admin yang ditambahkan lewat sini juga otomatis dianggap admin oleh
# is_admin(), jadi langsung bisa: buka /settings, pakai semua perintah
# admin lain, DAN membalas pesan live chat (lihat AdminReplyFilter).

async def addadmin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/addadmin <user_id> -- daftarkan user_id tsb sebagai admin baru."""
    if not await _group_command_allowed(update, context):
        return
    if not is_admin(update.effective_user.id):
        return
    ...
    if not context.args:
        await update.message.reply_text(
            ...
            "Tips: user_id bisa didapat dengan meminta calon admin mengirim "
            "pesan apa saja ke bot ini lalu forward pesan tersebut ke "
            "@userinfobot, atau lihat log bot.",
            parse_mode="Markdown",
        )
        return
    ...
    try:
        ...
        logger.warning(
            "Gagal ambil info profil user %s saat /addadmin (mungkin user itu "
            "belum pernah chat bot ini sama sekali).", new_admin_id,
        )

    db.add_bot_admin(new_admin_id, username, full_name, update.effective_user.id)
    ...
    try:
        ...
        logger.warning("Gagal mengatur daftar perintah admin untuk admin baru %s.", new_admin_id)

    display_name = f"@{username}" if username else (full_name or str(new_admin_id))
    await update.message.reply_text(
        f"✅ Berhasil menambahkan {display_name} (`{new_admin_id}`) sebagai admin.\n\n"
...
async def listadmin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/listadmin -- lihat daftar admin utama & admin tambahan."""
    if not await _group_command_allowed(update, context):
        return
    if not is_admin(update.effective_user.id):
        return
...
async def removeadmin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/removeadmin <user_id> -- hapus admin TAMBAHAN (bukan admin utama)."""
    if not await _group_command_allowed(update, context):
        return
    if not is_admin(update.effective_user.id):
        return
    ...
    if not context.args:
        await update.message.reply_text(
            "⚠️ Format salah.\nGunakan: `/removeadmin <user_id>`", parse_mode="Markdown"
        )
        return
