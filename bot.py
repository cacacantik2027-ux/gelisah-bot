...
def _admin_commands_list(public_commands):
    ...
    return public_commands + [
        ...
        BotCommand("linktalent", "Tautkan akun Telegram talent (reply pesan talent)"),
...
async def linktalent_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Cara pakai: balas (reply) pesan APAPUN dari talent yang bersangkutan,
    lalu jalankan:
      `/linktalent Nama Talent`
    Untuk melepas tautan (mis. talent resign / ganti akun), reply pesan
    talent itu juga lalu jalankan:
      `/linktalent hapus`"""
    if not is_admin(update.effective_user.id):
        return
    ...
    if not replied or not replied.from_user:
        await update.message.reply_text(
            "⚠️ Balas (reply) salah satu pesan DARI TALENT yang mau ditautkan, lalu jalankan ulang:\n"
            "`/linktalent Nama Talent`\n\n"
            "Untuk melepas tautan: reply pesan talent itu, lalu `/linktalent hapus`.",
            parse_mode="Markdown",
        )
        return
    ...
    if context.args and context.args[0].lower() == "hapus":
        _unlink_talent(target.id)
        await update.message.reply_text(
            f"✅ Tautan absen untuk {target.first_name} (id `{target.id}`) sudah dilepas.",
            parse_mode="Markdown",
        )
        return
...
def main():
    ...
    if app.job_queue:
        ...
        logger.warning(
            ...
            "auto-off proaktif DAN reset+reminder harian jam 06:00 WIB di topik Absen Talent "
            "tidak akan jalan. Install dengan: `pip install \"python-telegram-bot[job-queue]\"` "
            "kalau mau mengaktifkannya."
        )
