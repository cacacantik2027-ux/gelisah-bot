...
# Dipercepat & dibuat lebih halus: frame diganti lebih cepat (delay lebih
# kecil) dan durasi total thinking-dots dipersingkat, supaya balasan bot
# terasa lebih responsif tanpa kehilangan efek "sedang berpikir sebentar".
...
# Delay per langkah dipercepat (0.12s -> 0.045s) sekaligus jumlah langkah
# ditambah (lebih banyak potongan teks yang lebih kecil) -- hasilnya teks
# terungkap lebih cepat SEKALIGUS lebih mulus (langkahnya lebih halus),
# bukan cuma dipercepat kasar.
TYPEWRITER_STEP_DELAY = 0.045
TYPEWRITER_MIN_STEPS = 10
TYPEWRITER_MAX_STEPS = 26
...
async def send_thinking_reply(
    ...
    berhenti di teks FINAL yang sudah diformat lengkap. Kalau `reply_to_message_id`
    diisi, pesan (dan seluruh animasinya) tetap tampil sebagai balasan
    (reply) ke pesan tsb -- dipakai mis. saat bot di-mention di grup, supaya
    balasannya jelas ditujukan ke pesan user yang me-mention, bukan cuma
    dikirim lepas ke grup."""
    try:
        await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
    except Exception:
        pass
...
async def send_thinking_photo(
    ...
    `reply_to_message_id` (kalau diisi) membuat foto ini tampil sebagai
    balasan (reply) ke pesan tsb -- lihat catatan di send_thinking_reply()."""
    try:
        await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
    except Exception:
        pass
...
async def send_talent_card_to_chat(context, chat_id, talents, index, close_button=False, reply_to_message_id=None):
