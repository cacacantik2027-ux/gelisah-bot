...
def _testimoni_censor_available() -> bool:
    """True kalau pip package `pytesseract` TERPASANG *dan* binary
    `tesseract-ocr` di server-nya juga terdeteksi. Dipanggil sebelum
    _redact_sensitive_regions supaya kalau tidak tersedia, bot fallback
    kirim foto APA ADANYA (tanpa sensor) alih-alih error/gagal total."""
...
def _redact_sensitive_regions(image_bytes: bytes) -> bytes:
    ...
    "sensor nama pengirim dll kecuali nama penerima dan nominal".

    Ini heuristik OCR, BUKAN 100% akurat -- tata letak tiap
    bank/e-wallet beda-beda. Kalau OCR gagal baca apa pun / error, balikin
    bytes ASLI apa adanya (dijalankan dari _prepare_testimoni_photo dengan
    mode Approve Manual sebagai default, supaya admin sempat melihat hasil
    foto ASLI di grup live chat sebelum foto diproses & diposting)."""
    ...
    ordered = sorted(lines.values(), key=lambda l: (l["y0"], l["x0"]))
    for line in ordered:
        text = " ".join(line["words"])
        line["keep"] = bool(_NOMINAL_RE.search(text) or _RECIPIENT_LABEL_RE.search(text))
    for idx, line in enumerate(ordered):
        text = " ".join(line["words"])
        if _RECIPIENT_LABEL_RE.search(text) and idx + 1 < len(ordered):
...
async def _prepare_testimoni_photo(context: ContextTypes.DEFAULT_TYPE, file_id: str):
    ...
    if censor_on:
        if _testimoni_censor_available():
            ...
            logger.warning(
                "Sensor info sensitif testimoni AKTIF tapi tesseract-ocr tidak tersedia di server "
                "-- foto dikirim TANPA sensor. Install tesseract-ocr di server untuk mengaktifkan."
            )
...
def _testimoni_settings_text():
    ...
    censor_line = f"Sensor info sensitif: {'✅ ON' if censor else '❌ OFF'}"

    lines = ["📸 *Channel Testimoni*", "", status_line, mode_line, watermark_line, censor_line, ""]
    if censor and not _testimoni_censor_available():
        lines.append(
            "⚠️ Sensor sedang ON tapi *tesseract-ocr* belum terpasang di server -- foto akan "
            "tetap terkirim TANPA sensor sampai tesseract-ocr di-install di server.\n"
        )
    lines.append(
