...
async def handle_payment_proof(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    # Jika admin sedang mengedit field settings (termasuk QRIS), biarkan handler settings yang menangani
...
def main():
    ...
    # Handler global untuk input settings di luar conversation (setelah END/timeout)
    # WAJIB didaftarkan SEBELUM handle_payment_proof agar foto QRIS dari admin tidak salah tangkap
    app.add_handler(MessageHandler(filters.PHOTO | (filters.TEXT & ~filters.COMMAND), handle_settings_input_global))
