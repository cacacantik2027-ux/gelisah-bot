...
def talent_carousel_keyboard(talent, index, total, close_button=False, owner_id=None):
    ...
    rows = [
        [InlineKeyboardButton(
            talent["name"],
            callback_data=_owner_suffixed(f"talent_{talent['id']}", owner_id),
            style=STYLE_PRIMARY,
        )],
    ]
...
def talent_detail_keyboard(talent, owner_id=None):
    ...
    rows.append([InlineKeyboardButton(
        "💰 Pricelist", callback_data=_owner_suffixed(f"price_{talent['id']}", owner_id), style=STYLE_PRIMARY
    )])
    ...
    rows.append([InlineKeyboardButton(
        ...
        callback_data=_owner_suffixed("menu_talents", owner_id),
        style=STYLE_PRIMARY,
    )])
    return InlineKeyboardMarkup(rows)


def back_to_talent_keyboard(talent_id, owner_id=None):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(
            "⬅️ Kembali", callback_data=_owner_suffixed(f"talent_{talent_id}", owner_id), style=STYLE_PRIMARY
        )],
    ])
