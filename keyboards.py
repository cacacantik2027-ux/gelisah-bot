...
def settings_menu_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📢 Auto Promo Talent", callback_data="settings_autopromo", style=STYLE_PRIMARY)],
...
def back_to_settings_keyboard(back_callback="settings_back"):
    ...
    `back_callback` bisa diisi callback_data submenu lain (mis.
    "settings_autopromo") kalau langkah ini dibuka dari dalam submenu,
    supaya tombol Kembali membawa admin ke submenu itu, bukan langsung ke
    Menu Pengaturan utama."""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⬅️ Kembali", callback_data=back_callback, style=STYLE_PRIMARY)],
    ])


def preview_edit_keyboard(edit_callback, back_callback="settings_back"):
    ...
    `back_callback` sama seperti di back_to_settings_keyboard() di atas."""
    return InlineKeyboardMarkup([
...
def autopromo_menu_keyboard(interval_minutes, topics_count, max_topics):
    """Submenu '📢 Auto Promo Talent' di Menu Pengaturan -- pusat semua
    pengaturan posting promo talent ready (teks, jadwal, grup/topik tujuan,
    dan aksi posting/hapus manual), supaya admin tidak perlu lagi hafal
    command /setpromogrup, /setpromojadwal, /postpromo, /hapuspromo satu-satu.
    `interval_minutes` None/"" -- jadwal berulang sedang OFF."""
    jadwal_label = f"⏰ Jadwal: tiap {interval_minutes} menit" if interval_minutes else "⏰ Jadwal: OFF (atur di sini)"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✏️ Ubah Teks Promo", callback_data="settings_promotext", style=STYLE_PRIMARY)],
        [InlineKeyboardButton(jadwal_label, callback_data="settings_autopromo_jadwal", style=STYLE_PRIMARY)],
        [InlineKeyboardButton(f"📋 Grup/Topik Tujuan ({topics_count}/{max_topics})", callback_data="settings_autopromo_topics", style=STYLE_PRIMARY)],
        [InlineKeyboardButton("🚀 Posting Sekarang", callback_data="settings_autopromo_postnow", style=STYLE_SUCCESS)],
        [InlineKeyboardButton("🗑️ Hapus Pesan Promo Sekarang", callback_data="settings_autopromo_delnow", style=STYLE_DANGER)],
        [InlineKeyboardButton("⬅️ Kembali", callback_data="settings_back", style=STYLE_PRIMARY)],
    ])


def autopromo_topics_keyboard(topics):
    """Daftar grup/topik tujuan Auto Promo yang sudah terdaftar (lewat
    /setpromogrup, dijalankan di dalam grup tujuannya -- Bot API tidak
    memungkinkan bot mendaftarkan grup baru dari private chat tanpa
    dijalankan di dalam grup itu sendiri), tiap baris punya tombol hapus.
    `index` (posisi di list, bukan group_id mentah) dipakai di callback_data
    supaya aman dari karakter minus pada group_id (id grup Telegram negatif)."""
    rows = []
    for i, t in enumerate(topics):
        topic_label = t.get("topic_id") or "-"
        rows.append([InlineKeyboardButton(
            f"🗑️ Grup {t.get('group_id')} / topik {topic_label}",
            callback_data=f"autopromo_deltopic_{i}",
            style=STYLE_DANGER,
        )])
    rows.append([InlineKeyboardButton("⬅️ Kembali", callback_data="settings_autopromo", style=STYLE_PRIMARY)])
    return InlineKeyboardMarkup(rows)
