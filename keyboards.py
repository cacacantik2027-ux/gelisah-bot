...
def talent_detail_keyboard(talent):
    ...
    rows.append([InlineKeyboardButton("💬 Chat Sekarang", callback_data=f"chat_{talent['id']}")])
...
def back_to_talent_keyboard(talent_id):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("💬 Chat Sekarang", callback_data=f"chat_{talent_id}")],
...
def end_chat_keyboard(session_id):
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔴 Akhiri Sesi", callback_data=f"endchat_{session_id}")],
    ])
...
def addtalent_step_keyboard(back_callback=None):
    ...
    rows.append([InlineKeyboardButton("❌ Batalkan", callback_data="addtalent_cancel")])
    return InlineKeyboardMarkup(rows)
...
def sponsor_menu_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ Tambah Sponsor", callback_data="sponsor_add")],
        ...
        [InlineKeyboardButton("🗑️ Hapus Sponsor", callback_data="sponsor_del")],
        [InlineKeyboardButton("⬅️ Kembali", callback_data="settings_back")],
    ])
...
def delete_sponsor_keyboard(sponsors):
    ...
    for s in sponsors:
        label = s["name"] or f"Sponsor #{s['id']}"
        rows.append([InlineKeyboardButton(f"🗑️ {label}", callback_data=f"sponsordelconfirm_{s['id']}")])
