...
def order_keyboard(idx: int):
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✅ Konfirmasi Order", callback_data=f"confirm_order_{idx}"),
            InlineKeyboardButton("« Kembali", callback_data=f"talent_{idx}"),
        ],
    ])
    ...
    # ── Order ──
        ...
        await ctx.bot.send_message(
            ...
            text=(
                f"🛒 *Konfirmasi Order*\n\n"
                f"Talent: *{t['name']}*\n\n"
                f"Tekan *Konfirmasi Order* untuk mengirim permintaan ke admin."
            ),
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=order_keyboard(idx),
        )

    # ── Konfirmasi Order → notif ke admin ──
        ...
        await query.edit_message_text(
            f"✅ *Order berhasil dikirim!*\n\n"
            f"Talent: *{t['name']}*\n\n"
            f"Admin akan segera menghubungi kamu. Terima kasih! 🙏",
            parse_mode=ParseMode.MARKDOWN,
        )
