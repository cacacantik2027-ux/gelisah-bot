...
async def settings_get_value(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ *{label}* berhasil diperbarui!\n\nKetik /settings untuk membuka panel kembali.",
        parse_mode=ParseMode.MARKDOWN,
    )

    # ← FIX UTAMA: END conversation setelah simpan agar tidak stuck di state SETTINGS_MENU
    # Admin tinggal ketik /settings lagi untuk membuka panel fresh
    ctx.user_data.clear()
    return ConversationHandler.END
