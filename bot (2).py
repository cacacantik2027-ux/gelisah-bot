...
def load_settings() -> dict:
    ...
    defaults = {
        ...
        "channel_id": 0,  # ID Channel untuk auto-forward saat order di-approve
        "watermark_file_id": ""  # File ID logo watermark untuk ditempel di foto bukti transfer
    }
    if not os.path.exists("data"):
        os.makedirs("data")
...
def load_users() -> dict:
    """Memuat database pengguna dari users.json"""
    if not os.path.exists("data"):
        os.makedirs("data")
    ...
    try:
        ...
        logger.error(f"Gagal membaca users.json: {e}")
        return {}

def save_users(users: dict):
    """Menyimpan database pengguna ke users.json"""
    if not os.path.exists("data"):
        os.makedirs("data")
    try:
        ...
        logger.error(f"Gagal menyimpan users.json: {e}")

def register_user(user_id: int, username: str = None, full_name: str = None):
    """Mendaftarkan atau memperbarui data pengguna di database"""
    users = load_users()
    uid = str(user_id)
    if uid not in users:
        users[uid] = {
            ...
            "joined": __import__("datetime").datetime.now().isoformat()
        }
        save_users(users)
    else:
        # Update info terbaru jika ada perubahan nama/username
        changed = False
        if users[uid].get("username") != username:
            users[uid]["username"] = username
            changed = True
        if users[uid].get("full_name") != full_name:
            users[uid]["full_name"] = full_name
            changed = True
        # Jika sebelumnya diblokir lalu aktif lagi
        if users[uid].get("status") == "blocked":
...
def get_user_stats() -> dict:
    ...
    blocked = sum(1 for u in users.values() if u.get("status") == "blocked")
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    # Catat/daftarkan user ke database
...
async def handle_payment_proof(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    # ── Prioritas 1: Admin sedang input settings (termasuk upload foto QRIS) ──
    if ctx.user_data.get("editing_field"):
        # Langsung proses sebagai input settings, tidak lanjut ke payment proof
        await handle_settings_input_global(update, ctx)
        return
    # ── Prioritas 2: User sedang menunggu konfirmasi bukti transfer ──
    ...
    # ── FIX: escape karakter spesial Markdown agar tidak gagal parsing ──
    # Karakter _ * [ ] ( ) ~ ` > # + - = | { } . ! adalah reserved di MarkdownV2,
    # tapi untuk parse_mode Markdown (v1) yang rawan adalah _ * ` [
    ...
    safe_name = esc_md(user.full_name or "Tanpa Nama")
    safe_username = esc_md(user.username or "-")
    safe_talent_name = esc_md(talent.get("name", "-"))

    admin_caption = (
        f"🔔 *BUKTI PEMBAYARAN BARU!*\n\n"
        f"🙋‍♂️ *User:* [{safe_name}](tg://user?id={user.id})\n"
        f"🆔 *Username:* @{safe_username}\n"
        f"🆔 *ID User:* `{user.id}`\n\n"
        f"👤 *Talent Dipesan:* {safe_talent_name}\n\n"
    ...
    try:
        ...
        logger.error(f"Gagal mengirim log ke grup (markdown): {e}")
        # ── FIX: fallback tanpa parse_mode kalau Markdown tetap gagal ──
        try:
            plain_caption = (
                f"🔔 BUKTI PEMBAYARAN BARU!\n\n"
                f"🙋‍♂️ User: {user.full_name or 'Tanpa Nama'} (id: {user.id})\n"
                f"🆔 Username: @{user.username or '-'}\n\n"
                f"👤 Talent Dipesan: {talent.get('name', '-')}\n\n"
                f"Tolong cek mutasi dan konfirmasi di bawah 👇"
            )
            await ctx.bot.send_photo(
                chat_id=log_group_id,
            ...
            logger.error(f"Gagal mengirim log ke grup (plain fallback): {e2}")
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "set_back_to_menu":
        ...
        ch_esc      = _html.escape(str(new_settings.get("channel_id", 0)))
        msg = (
            ...
            f"📢 <b>Channel ID:</b> <code>{ch_esc}</code>\n\n"
    ...
    # Tangani tombol menu settings yang diklik di luar conversation (setelah END/timeout)
    # Ini memastikan panel settings selalu bisa digunakan ulang tanpa harus /settings lagi
    settings_fields_map = {
        ...
        "set_channel": ("channel_id",   "Kirimkan *ID Channel* Telegram untuk auto-forward saat order di-approve.\n\nPastikan:\n1. Bot sudah dijadikan *Admin* di channel tersebut\n2. Format ID berawalan minus (contoh: `-100123456789`)\n\nKirim `0` untuk menonaktifkan fitur ini."),
    }
    if data in settings_fields_map:
        if not is_admin(query.from_user.id):
            await query.answer("⛔ Akses ditolak.", show_alert=True)
            return
        field, prompt = settings_fields_map[data]
    ...
    # ── Tombol Approve / Tolak Pesanan dari Admin di Grup Log ──
    if data.startswith("acc_") or data.startswith("rej_"):
        ...
        if action == "acc":
            # ── Simpan semua data SEBELUM apapun diubah ──
            proof_photo_id = query.message.photo[-1].file_id if query.message.photo else None
            original_caption = query.message.caption or ""
            channel_id = int(settings.get("channel_id", 0))
            talent_obj = talents[t_idx] if t_idx < len(talents) else None
            talent_photo = talent_obj.get("photo") if talent_obj else None
            watermark_file_id = settings.get("watermark_file_id", "")

            logger.info(f"[APPROVE] proof_photo_id={proof_photo_id}, talent_photo={talent_photo}, channel_id={channel_id}")

            # ── Edit caption pesan di grup log ──
            ...
            try:
                ...
                logger.error(f"Gagal edit caption grup log: {e}")
            ...
            try:
                ...
                logger.error(f"Gagal kirim notifikasi ke user {user_id}: {e}")

            # ── Auto-post ke channel ──
            if channel_id != 0:
                # Gunakan HTML agar username dengan underscore tidak konflik dengan Markdown italic
                import html as _html
                talent_name_esc = _html.escape(talent_name)
                caption_testi = (
                    f"✅ <b>TESTIMONI {talent_name_esc}</b>\n\n"
                    "Order Sekarang : @ordervcsgs_bot"
                )
                try:
                    # Proses watermark jika ada
                    if proof_photo_id and watermark_file_id:
                        try:
                            watermarked = await apply_watermark(ctx.bot, proof_photo_id, watermark_file_id)
                            proof_media = InputMediaPhoto(media=watermarked)
                        except Exception as wm_err:
                            logger.error(f"Gagal apply watermark: {wm_err}")
                            proof_media = InputMediaPhoto(media=proof_photo_id)
                    elif proof_photo_id:
                        proof_media = InputMediaPhoto(media=proof_photo_id)
                    else:
                        proof_media = None

                    logger.info(f"[AUTOPOST] proof_media={'ada' if proof_media else 'kosong'}, talent_photo={'ada' if talent_photo else 'kosong'}")

                    if proof_media and talent_photo:
                        # Album 2 foto: foto talent (dengan caption) + foto bukti transfer
                        talent_media = InputMediaPhoto(
                            media=talent_photo,
                            caption=caption_testi,
                            parse_mode=ParseMode.HTML,
                        )
                        await ctx.bot.send_media_group(
                            chat_id=channel_id,
                            media=[talent_media, proof_media],
                        )
                        logger.info(f"[AUTOPOST] Album berhasil dikirim ke channel {channel_id}")
                    elif proof_media:
                        await ctx.bot.send_photo(
                            chat_id=channel_id,
                        ...
                        logger.info(f"[AUTOPOST] Hanya bukti transfer dikirim ke channel {channel_id}")
                    elif talent_photo:
                        await ctx.bot.send_photo(
                            chat_id=channel_id,
                            photo=talent_photo,
                        ...
                        logger.info(f"[AUTOPOST] Hanya foto talent dikirim ke channel {channel_id}")
                    else:
                        logger.warning("[AUTOPOST] Tidak ada foto yang bisa diposting ke channel")
                except Exception as e:
                    logger.error(f"[AUTOPOST] Gagal posting ke channel {channel_id}: {e}", exc_info=True)
            else:
                logger.info("[AUTOPOST] channel_id=0, fitur autopost tidak aktif")
...
async def admin_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "adm_close":
        ...
        await query.edit_message_text(
            "➕ *Tambah Talent Baru*\n\nKirim *nama* talent:",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Kembali", callback_data="add_back_to_menu")]]),
        )
        return ADD_NAME
        ...
        await query.edit_message_text(
            f"✏️ Kirim {prompt}:",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Kembali", callback_data=f"edit_pick_{idx}")]]),
        )
        return EDIT_VALUE
...
async def admin_add_cancel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Tombol Kembali di langkah manapun proses tambah talent"""
    query = update.callback_query
    await query.answer()
    step = query.data  # add_back_to_name / add_back_to_photo / add_back_to_desc / add_back_to_menu

    if step == "add_back_to_menu":
    ...
    elif step == "add_back_to_photo":
        await query.edit_message_text(
            "📷 Kirim *foto* talent (atau ketik `skip`):",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Kembali", callback_data="add_back_to_name")]]),
        )
        return ADD_PHOTO

    elif step == "add_back_to_desc":
        await query.edit_message_text(
            "📄 Kirim *deskripsi* talent:",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Kembali", callback_data="add_back_to_photo")]]),
        )
        return ADD_DESC
...
async def admin_get_desc(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Kembali", callback_data="add_back_to_desc")]]),
    )
    return ADD_PRICE
...
async def admin_edit_back(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Tombol Kembali di input nilai edit — kembali ke halaman pilih field talent"""
    query = update.callback_query
    await query.answer()
    idx = ctx.user_data.get("edit_idx", 0)
    talents = load_talents()
...
async def admin_edit_value(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Tombol kembali ke halaman pilih field talent yang sama
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("« Kembali ke Pilih Field", callback_data=f"edit_pick_{idx}")],
        [InlineKeyboardButton("« Menu Admin", callback_data="adm_back")],
        [InlineKeyboardButton("❌ Tutup", callback_data="adm_close")],
    ])
...
async def apply_watermark(bot, photo_file_id: str, watermark_file_id: str) -> io.BytesIO:
    """Download foto + watermark, tempel watermark di tengah, return BytesIO"""
    try:
        from PIL import Image
    except ImportError:
        raise RuntimeError("Pillow belum terinstall. Tambahkan 'Pillow' ke requirements.txt")
    ...
    # Download watermark
    wm_file = await bot.get_file(watermark_file_id)
    wm_bytes = await wm_file.download_as_bytearray()
    wm_img = Image.open(io.BytesIO(wm_bytes)).convert("RGBA")

    # Resize watermark: 50% lebar foto, proporsional
    base_w, base_h = base_img.size
    wm_target_w = int(base_w * 0.5)
    ...
    # Buat layer transparan 50%
    wm_with_alpha = wm_img.copy()
    r, g, b, a = wm_with_alpha.split()
    a = a.point(lambda x: int(x * 0.5))
    wm_with_alpha = Image.merge("RGBA", (r, g, b, a))

    # Tempel di tengah
    pos_x = (base_w - wm_target_w) // 2
    pos_y = (base_h - wm_target_h) // 2
    base_img.paste(wm_with_alpha, (pos_x, pos_y), wm_with_alpha)

    # Konversi ke JPEG BytesIO
    output = io.BytesIO()
    base_img.convert("RGB").save(output, format="JPEG", quality=92)
    output.seek(0)
    return output

async def setwatermark_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Admin upload logo watermark — bisa kirim foto langsung atau reply ke foto"""
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("⛔ Akses ditolak.")
        return
    ...
    # Cara 1: foto dikirim bersamaan perintah
    if update.message.photo:
    ...
    # Cara 2: perintah sebagai reply ke foto
    if not file_id and update.message.reply_to_message:
    ...
    if not file_id:
        await update.message.reply_text(
            "🖼 *Cara set watermark:*\n\n"
            "1️⃣ Kirim foto/logo bersamaan dengan perintah:\n"
            "   Lampirkan gambar → isi caption `/setwatermark`\n\n"
            "2️⃣ Atau kirim gambar dulu → lalu *balas* gambar tersebut dengan `/setwatermark`\n\n"
            "Ketik `/delwatermark` untuk menghapus watermark.",
            parse_mode=ParseMode.MARKDOWN
        )
        return

    settings = load_settings()
    settings["watermark_file_id"] = file_id
    save_settings(settings)

    await update.message.reply_text(
        "✅ *Watermark berhasil disimpan!*\n\n"
        "Logo akan otomatis ditempel di tengah foto bukti transfer saat posting ke channel.",
        parse_mode=ParseMode.MARKDOWN
    )

async def delwatermark_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Hapus watermark dari settings"""
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("⛔ Akses ditolak.")
        return
    settings = load_settings()
    ...
    save_settings(settings)
    await update.message.reply_text("✅ Watermark berhasil dihapus. Foto bukti transfer tidak akan diberi watermark lagi.")

async def cekwatermark_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Menampilkan watermark yang tersimpan saat ini"""
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("⛔ Akses ditolak.")
        return

    settings = load_settings()
    file_id = settings.get("watermark_file_id", "")

    if not file_id:
        await update.message.reply_text(
            "ℹ️ *Belum ada watermark tersimpan.*\n\n"
            "Gunakan `/setwatermark` untuk mengatur watermark.",
            parse_mode=ParseMode.MARKDOWN
        )
        return
    ...
    try:
        await update.message.reply_photo(
            photo=file_id,
            caption=(
                "🖼 *Watermark Aktif Saat Ini*\n\n"
                f"📎 File ID:\n`{file_id}`\n\n"
                "Watermark ini ditempel di tengah foto bukti transfer saat autopost.\n\n"
                "Gunakan /delwatermark untuk menghapusnya."
            ),
            parse_mode=ParseMode.MARKDOWN
        )
    except Exception as e:
        await update.message.reply_text(
            f"⚠️ Watermark tersimpan tapi gagal ditampilkan.\n\n"
            f"📎 File ID:\n`{file_id}`\n\n"
            f"Error: `{e}`",
            parse_mode=ParseMode.MARKDOWN
        )
...
async def stats_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Menampilkan statistik pengguna bot"""
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("⛔ Akses ditolak.")
        return
    ...
    msg = (
        "📊 *STATISTIK PENGGUNA BOT GELISAH*\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        f"✅ Pengguna Aktif      : *{stats['active']}* orang\n"
        f"🚫 Blokir Bot          : *{stats['blocked']}* orang\n"
        f"👥 Total Pengguna      : *{stats['total']}* orang\n\n"
        "━━━━━━━━━━━━━━━━━━━━"
    )
    await update.message.reply_text(msg, parse_mode=ParseMode.MARKDOWN)

async def broadcast_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Mengirim pesan broadcast ke semua pengguna aktif dengan animasi persentase"""
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("⛔ Akses ditolak.")
        return
    ...
    pesan = " ".join(ctx.args)
    users = load_users()
    active_users = [(uid, u) for uid, u in users.items() if u.get("status") == "active"]
    total_active = len(active_users)

    if total_active == 0:
        await update.message.reply_text("⚠️ Belum ada pengguna aktif yang terdaftar.")
        return

    # Pesan status awal dengan animasi bar
    def build_bar(persen: int) -> str:
    ...
    for i, (uid, udata) in enumerate(active_users):
        try:
            await ctx.bot.send_message(
                chat_id=int(uid),
                text=f"📢 *Pesan dari Admin GELISAH:*\n\n{pesan}",
                parse_mode=ParseMode.MARKDOWN
            )
            terkirim += 1
        except Exception as e:
        ...
        # Update animasi setiap 5 user atau di akhir
        persen = int(((i + 1) / total_active) * 100)
        if (i + 1) % 5 == 0 or (i + 1) == total_active:
            try:
                await status_msg.edit_text(
                    f"📡 *Broadcast Berjalan...*\n\n"
                    f"`{build_bar(persen)}` {persen}%\n\n"
                    f"✅ Terkirim : {terkirim}\n"
                    f"❌ Gagal    : {gagal}",
                    parse_mode=ParseMode.MARKDOWN
                )
    ...
    # Update status blocked di database
    if blocked_ids:
        users_db = load_users()
        for bid in blocked_ids:
            if bid in users_db:
                users_db[bid]["status"] = "blocked"
        save_users(users_db)

    # Laporan akhir broadcast
    await status_msg.edit_text(
        f"✅ *Broadcast Selesai!*\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📨 Pesan Dikirim ke  : *{total_active}* pengguna aktif\n"
        f"✅ Berhasil Terkirim : *{terkirim}* orang\n"
        f"🚫 Blokir Bot        : *{len(blocked_ids)}* orang\n"
        f"❌ Gagal Lainnya     : *{gagal - len(blocked_ids)}* orang\n\n"
        f"━━━━━━━━━━━━━━━━━━━━",
        parse_mode=ParseMode.MARKDOWN
    )
...
async def settings_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    channel_esc = html.escape(str(settings.get("channel_id", 0)))
    ...
    msg = (
        ...
        f"📢 <b>Channel ID:</b> <code>{channel_esc}</code>\n\n"
        "Pilih konfigurasi yang ingin Anda ubah di bawah ini:"
    )
...
async def settings_get_value(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if field == "channel_id":
        try:
            ...
            await update.message.reply_text("❌ Format ID salah. Harus berupa angka (contoh: `-100123456789`). Coba kirim ulang:")
            return SETTINGS_INPUT
...
async def handle_settings_input_global(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """
    Menangkap input teks/foto untuk settings saat conversation sudah END.
    Dipanggil hanya jika ctx.user_data['editing_field'] masih ada (sisa dari callback global).
    """
    field = ctx.user_data.get("editing_field")
    if not field:
        # Bukan sesi settings, abaikan — biarkan handler lain yang menangani
        return
    ...
    if field == "log_group_id":
        try:
            ...
            await update.message.reply_text("❌ Format ID salah. Harus berupa angka bulat negatif (contoh: `-100123456789`). Coba kirim ulang:", parse_mode=ParseMode.MARKDOWN)
            return
    ...
    if field == "channel_id":
        try:
            ...
            await update.message.reply_text("❌ Format ID salah. Harus berupa angka (contoh: `-100123456789`). Coba kirim ulang:", parse_mode=ParseMode.MARKDOWN)
            return
...
def main():
    ...
    admin_conv = ConversationHandler(
        ...
        states={
            ...
            ADD_NAME:     [CallbackQueryHandler(admin_add_cancel, pattern="^add_back_to_menu$"), MessageHandler(filters.TEXT & ~filters.COMMAND, admin_get_name)],
            ADD_PHOTO:    [CallbackQueryHandler(admin_add_cancel, pattern="^add_back_to_(menu|name)$"), MessageHandler(filters.PHOTO | (filters.TEXT & ~filters.COMMAND), admin_get_photo)],
            ADD_DESC:     [CallbackQueryHandler(admin_add_cancel, pattern="^add_back_to_(menu|name|photo)$"), MessageHandler(filters.TEXT & ~filters.COMMAND, admin_get_desc)],
            ADD_PRICE:    [CallbackQueryHandler(admin_add_cancel, pattern="^add_back_to_(menu|name|photo|desc)$"), MessageHandler(filters.TEXT & ~filters.COMMAND, admin_get_price)],
            EDIT_CHOOSE:  [CallbackQueryHandler(admin_callback)],
            EDIT_FIELD:   [CallbackQueryHandler(admin_callback)],
            EDIT_VALUE:   [CallbackQueryHandler(admin_edit_back, pattern="^edit_pick_"), MessageHandler(filters.PHOTO | (filters.TEXT & ~filters.COMMAND), admin_edit_value)],
    ...
    settings_conv = ConversationHandler(
        ...
        fallbacks=[
            ...
            # Fallback untuk tombol yang stuck setelah timeout — ditangani callback_handler global
            CallbackQueryHandler(settings_back_to_menu, pattern="^set_back_to_menu$"),
            CallbackQueryHandler(lambda u, c: u.callback_query.answer() or None, pattern="^set_close$"),
        ],
        per_message=False,
        allow_reentry=True,
        # Timeout dihapus — penyebab bug stuck tombol setelah 5 menit idle
        # Tombol lama tetap bisa diklik kapan saja via callback_handler global sebagai fallback
    )
    ...
    app.add_handler(CommandHandler("setwatermark", setwatermark_cmd))
    app.add_handler(CommandHandler("delwatermark", delwatermark_cmd))
    app.add_handler(CommandHandler("cekwatermark", cekwatermark_cmd))
    
    # Mengaktifkan Form Alur Logika Percakapan
    app.add_handler(admin_conv)
    app.add_handler(settings_conv)

    # Handler foto tunggal: menangani QRIS settings (admin) dan bukti transfer (user) sekaligus
    # Logika prioritas ada di dalam fungsi handle_payment_proof itu sendiri
    app.add_handler(MessageHandler(filters.PHOTO, handle_payment_proof))

    # Handler teks global untuk input settings di luar conversation (teks saja, foto sudah di atas)
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_settings_input_global))
