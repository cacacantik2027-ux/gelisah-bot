...
async def admin_add_cancel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Tombol Kembali di tengah proses tambah talent — buang data sementara, kembali ke menu admin"""
    query = update.callback_query
    await query.answer()
...
async def admin_get_name(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Kembali", callback_data="adm_add_cancel")]]),
    )
    return ADD_PHOTO
...
def main():
    ...
    admin_conv = ConversationHandler(
        ...
        states={
            ...
            ADD_NAME:     [CallbackQueryHandler(admin_add_cancel, pattern="^adm_add_cancel$"), MessageHandler(filters.TEXT & ~filters.COMMAND, admin_get_name)],
            ADD_PHOTO:    [CallbackQueryHandler(admin_add_cancel, pattern="^adm_add_cancel$"), MessageHandler(filters.PHOTO | (filters.TEXT & ~filters.COMMAND), admin_get_photo)],
            ADD_DESC:     [CallbackQueryHandler(admin_add_cancel, pattern="^adm_add_cancel$"), MessageHandler(filters.TEXT & ~filters.COMMAND, admin_get_desc)],
            ADD_PRICE:    [CallbackQueryHandler(admin_add_cancel, pattern="^adm_add_cancel$"), MessageHandler(filters.TEXT & ~filters.COMMAND, admin_get_price)],
