...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Tombol Batalkan Pesanan Sesi Selesai (Bekerja Sempurna) ──
        ...
        # Menghapus total sesi saat pengguna kembali ke menu utama
        ctx.user_data.clear()
        try:
            await query.message.delete()
        except Exception:
...
async def admin_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "adm_close":
        ...
        ctx.user_data.clear() # Bersihkan sesi sesudah menghapus
        return ConversationHandler.END
...
async def admin_get_price(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data.clear() # Sesi pembuatan talent selesai, bersihkan total
    return ConversationHandler.END
...
async def admin_edit_value(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data.clear() # Sesi edit selesai, bersihkan total
    return ConversationHandler.END
...
async def settings_get_value(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Mengembalikan menu settings utama kembali dengan data baru (format aman HTML)
    new_settings = load_settings()
    welcome_esc = html.escape(new_settings.get("welcome_text", ""))
    bank_esc = html.escape(new_settings.get("bank_info", ""))
    qris_esc = html.escape(str(new_settings.get("qris_url", "")))
    livechat_esc = html.escape(new_settings.get("livechat_bot", ""))
    log_group_esc = html.escape(str(new_settings.get("log_group_id", "")))
    ...
    try:
        ...
        fallback_msg = (
            "⚙️ PENGATURAN BOT GELISAH (FALLBACK TEXT)\n\n"
            f"Welcome Text: {new_settings.get('welcome_text')}\n"
            f"Info Bank: {new_settings.get('bank_info')}\n"
            f"QRIS: {new_settings.get('qris_url')}\n"
