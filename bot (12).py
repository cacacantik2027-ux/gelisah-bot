...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("acc_") or data.startswith("rej_"):
        ...
        if action == "acc":
            ...
            if channel_id != 0:
                try:
                    talent = talents[t_idx] if t_idx < len(talents) else None
                    talent_photo = talent.get("photo") if talent else None
                    ...
                    # Kirim sebagai album (media group) jika ada foto talent
                    if proof_media and talent_photo:
                        ...
                        # Hanya foto bukti transfer tanpa foto talent
                        await ctx.bot.send_photo(
                            chat_id=channel_id,
                        ...
                        # Hanya foto talent tanpa bukti transfer
                        await ctx.bot.send_photo(
                            chat_id=channel_id,
                            photo=talent_photo,
                            caption=caption_testi,
                    ...
                    logger.error(f"Gagal forward ke channel: {e}")
...
async def setwatermark_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Admin upload logo watermark lewat perintah /setwatermark"""
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("⛔ Akses ditolak.")
        return

    # Cek apakah ada foto/stiker yang dikirim bersamaan
    ...
    if not file_id:
        await update.message.reply_text(
            "🖼 *Cara set watermark:*\n\n"
            "Kirim foto/logo/stiker bersamaan dengan perintah ini:\n"
            "`/setwatermark` + lampirkan gambar\n\n"
            "Atau kirim gambar dulu, lalu balas gambar tersebut dengan `/setwatermark`\n\n"
...
async def setwatermark_reply_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Handle /setwatermark sebagai reply ke foto"""
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("⛔ Akses ditolak.")
        return
    ...
    if not reply:
        await update.message.reply_text(
            "⚠️ Balas sebuah foto/gambar dengan perintah ini, atau kirim `/setwatermark` bersamaan dengan foto.",
            parse_mode=ParseMode.MARKDOWN
        )
        return
    ...
    if not file_id:
        await update.message.reply_text("❌ Pesan yang dibalas bukan foto atau gambar.")
        return
...
def main():
    ...
    app.add_handler(CommandHandler("setwatermark", setwatermark_reply_cmd))
