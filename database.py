...
# Kolom group_admins yang boleh diedit satu-per-satu lewat menu "✏️ Edit Admin".
_GROUP_ADMIN_EDITABLE_FIELDS = {"username", "full_name", "jabatan", "photo_file_id"}


def update_group_admin_field(admin_id, field, value):
    """Ubah SATU kolom milik kartu admin grup tertentu. `field` divalidasi
    terhadap whitelist supaya tidak bisa dipakai untuk menyuntik nama kolom
    sembarangan (mirip update_talent_field/update_sponsor_field)."""
    if field not in _GROUP_ADMIN_EDITABLE_FIELDS:
        raise ValueError(f"Field admin grup tidak dikenali: {field}")
    with get_conn() as conn:
        conn.execute(f"UPDATE group_admins SET {field} = ? WHERE id = ?", (value, admin_id))
        conn.commit()


# ---------- Admin Bot (ditambahkan lewat /addadmin) ----------
# Admin "utama" tetap didaftarkan lewat config.ADMIN_IDS (env var di server).
# Tabel ini menyimpan admin TAMBAHAN yang didaftarkan lewat perintah
# /addadmin oleh admin yang sudah ada, supaya bisa ikut mengelola & membalas
# live chat tanpa perlu ubah konfigurasi server / redeploy.

def add_bot_admin(user_id, username, full_name, added_by):
    """Tambah admin bot baru, atau perbarui datanya kalau user_id sudah
    pernah ditambahkan sebelumnya."""
    with get_conn() as conn:
        conn.execute(
            "INSERT INTO bot_admins (user_id, username, full_name, added_by) "
            "VALUES (?, ?, ?, ?) "
            "ON CONFLICT(user_id) DO UPDATE SET "
            "username=excluded.username, full_name=excluded.full_name, "
...
def list_bot_admins():
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM bot_admins ORDER BY created_at ASC"
        ).fetchall()
        return [dict(r) for r in rows]
...
# ---- Fitur Absen Talent (ready/tidak-ready) ----

def set_talent_status(talent_id, is_ready, ready_until, raw_text, updated_by):
    """Upsert status ready/tidak-ready satu talent. `ready_until` -- string
    UTC naive format 'YYYY-MM-DD HH:MM:SS' (konsisten dgn CURRENT_TIMESTAMP
    kolom lain di skema ini), atau None kalau ready tanpa batas waktu."""
    with get_conn() as conn:
        conn.execute("""
            INSERT INTO talent_status (talent_id, is_ready, ready_until, raw_text, updated_by, updated_at)
        ...
        """, (talent_id, int(is_ready), ready_until, raw_text, updated_by))
        conn.commit()


def get_talent_status(talent_id):
    with get_conn() as conn:
        row = conn.execute(
            "SELECT * FROM talent_status WHERE talent_id = ?", (talent_id,)
        ).fetchone()
        return dict(row) if row else None


def list_talent_statuses():
    with get_conn() as conn:
        rows = conn.execute("SELECT * FROM talent_status").fetchall()
        return [dict(r) for r in rows]


def expire_talent_statuses(now_utc_str):
    """Set is_ready=0 untuk semua talent yang sedang ready TAPI ready_until-nya
    sudah lewat `now_utc_str` (string UTC 'YYYY-MM-DD HH:MM:SS', dibanding
    sebagai string -- aman karena formatnya fixed-width & sortable).
    Balikin list talent_id yang baru saja di-expire (dipakai buat kirim
    notifikasi auto-off, lihat _check_talent_status_expiry di bot.py)."""
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT talent_id FROM talent_status "
            "WHERE is_ready = 1 AND ready_until IS NOT NULL AND ready_until <= ?",
            (now_utc_str,),
        ).fetchall()
        ids = [r["talent_id"] for r in rows]
        if ids:
            conn.execute(
                f"UPDATE talent_status SET is_ready = 0 WHERE talent_id IN ({','.join('?' * len(ids))})",
                ids,
            )
            conn.commit()
        return ids
