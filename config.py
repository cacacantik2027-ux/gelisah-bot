...
BOT_NAME = os.getenv("BOT_NAME", "Bot Official Gelisah")
...
# Short name Mini App yang didaftarkan lewat @BotFather -> /newapp (bagian setelah
# username bot di link https://t.me/<username_bot>/<short_name>). Dipakai untuk
# tombol Mini App di channel lewat command /postkatalog.
WEBAPP_SHORT_NAME = os.getenv("WEBAPP_SHORT_NAME", "katalog")

# Custom emoji ID (opsional) yang ditampilkan di depan teks tombol /postkatalog.
# Kosongkan kalau tidak dipakai. Ambil ID-nya dengan forward pesan berisi emoji
# custom itu ke @userinfobot / @RawDataBot, lihat field `custom_emoji_id`.
# CATATAN: khusus tombol di channel, ini hanya tampil kalau bot sudah beli
# username tambahan di Fragment (lihat penjelasan di keyboards.py).
CHANNEL_BUTTON_ICON_EMOJI_ID = os.getenv("CHANNEL_BUTTON_ICON_EMOJI_ID", "") or None
